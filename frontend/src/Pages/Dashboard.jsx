...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        // Filter resumes that belong to the logged-in user
        const userResumes = storedResumes.filter(
            (resume) => resume.userEmail === currentUserEmail
        );

        setResumes(userResumes.reverse()); // newest first
    }, []);
    ...
    const handleDeleteResume = (indexToDelete) => {
        ...
        // Keep only this user's resumes
        const userResumes = stored.filter((r) => r.userEmail === currentUserEmail);

        const reversedIndex = userResumes.length - 1 - indexToDelete;
        userResumes.splice(reversedIndex, 1);

        // Update the master resumes list in localStorage
        const updatedAllResumes = stored.filter(
            (r) => r.userEmail !== currentUserEmail
        ).concat(userResumes);

        localStorage.setItem("resumes", JSON.stringify(updatedAllResumes));
        setResumes(userResumes.reverse());
        if (openEmailIndex === indexToDelete) setOpenEmailIndex(null);
    };
                ...
                {resumes.length === 0 ? (
                    <div className="bg-white/80 backdrop-blur-sm border border-slate-200 rounded-2xl p-12 text-center">
                        ...
                        <p className="text-slate-400 font-light tracking-wide text-lg mb-2">
                            NO RESUMES FOUND FOR YOUR ACCOUNT
                        </p>
                        ...
                        <button
                            onClick={() => navigate("/info")}
                            className="mt-2 px-6 py-2 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-xl text-sm tracking-wide shadow-md hover:shadow-lg transition-all"
                        >
                            CREATE RESUME
                        </button>
                    </div>
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <h2 className="text-base font-medium text-slate-700 tracking-wide">
                                                {r.resume_json?.generated_title || r.resume_json?.header?.title || "Untitled Resume"}
